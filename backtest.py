...
TAKE_PROFIT_PCT     = 0.10   # 10% favourable move
POSITION_SIZE_USD   = 1000

# If more than this many tickers signal the same direction on one day = macro event
...
def compute_signals(ticker: str, date: datetime, history: pd.DataFrame):
    ...
    long_sig  = BTSignal(ticker, date.strftime("%Y-%m-%d"), "long",  long_score,  lt, False) \
                if long_score  >= COMPOSITE_SCORE_MIN and len(lt) >= MIN_SIGNALS else None
...
def simulate_trade(ticker: str, direction: str, entry_date: datetime,
    ...
    if direction == "long":
        stop_p   = entry_price * (1 - STOP_LOSS_PCT)
        target_p = entry_price * (1 + TAKE_PROFIT_PCT)
    else:
        stop_p   = entry_price * (1 + STOP_LOSS_PCT)   # loss if price rises
        target_p = entry_price * (1 - TAKE_PROFIT_PCT) # profit if price falls
    ...
    pnl_usd       = round(POSITION_SIZE_USD * pnl_pct / 100, 2)
...
def run_backtest(days_back: int = 30, long_only: bool = False, short_only: bool = False,
    ...
    log.info(f"Parameters: score>={COMPOSITE_SCORE_MIN}, signals>={MIN_SIGNALS}, "
             f"hold={HOLD_DAYS}d, SL={STOP_LOSS_PCT*100:.0f}%, TP={TAKE_PROFIT_PCT*100:.0f}%")
    log.info("=" * 65)
    ...
    for day in trading_days:
        ...
        for ticker in WATCHLIST:
            ...
            if long_sig  and "long"  in sides: day_longs.append((ticker, long_sig))
            if short_sig and "short" in sides: day_shorts.append((ticker, short_sig))
        ...
        for ticker, signal in day_longs:
            if ticker in open_long and (day - open_long[ticker]).days < HOLD_DAYS:
                continue
            ...
            trade = simulate_trade(ticker, "long", ed, ep, all_history[ticker], signal)
            all_trades.append(asdict(trade))
            open_long[ticker] = day
            day_trades.append((trade, signal))

        for ticker, signal in day_shorts:
            if ticker in open_short and (day - open_short[ticker]).days < HOLD_DAYS:
                continue
            future = all_history[ticker][all_history[ticker].index > pd.Timestamp(day)]
            if future.empty: continue
            ep = float(future.iloc[0]["Open"])
            ed = future.index[0].to_pydatetime()
            trade = simulate_trade(ticker, "short", ed, ep, all_history[ticker], signal)
            all_trades.append(asdict(trade))
