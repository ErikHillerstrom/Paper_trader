...
# ── Long parameters ──
LONG_SCORE_MIN       = 0.50   # minimum composite score to open a long
LONG_MIN_SIGNALS     = 1      # minimum signals triggered
LONG_HOLD_DAYS       = 3      # days to hold before time-exit
LONG_STOP_LOSS_PCT   = 0.05   # stop out if price drops this much
LONG_TAKE_PROFIT_PCT = 0.10   # take profit if price rises this much

# ── Short parameters (tuned independently) ──
SHORT_SCORE_MIN      = 0.70   # higher bar - shorts need stronger confirmation
SHORT_MIN_SIGNALS    = 2      # require at least 2 signals for shorts
SHORT_HOLD_DAYS      = 2      # shorter hold - shorts can reverse fast
SHORT_STOP_LOSS_PCT  = 0.05   # stop out if price rises this much
...
MAX_POSITIONS        = 17      # maximum simultaneous open slots
COMPOUNDING         = False    # True = position size grows/shrinks with equity
...
def run_backtest(days_back: int = 30, long_only: bool = False, short_only: bool = False,
    ...
    for day in trading_days:
        ...
        portfolio = [p for p in portfolio if p["exit_date"] > day]
        ...
        for ticker, signal, direction in all_candidates:
            ...
            # Track open position for slot management
            hold = LONG_HOLD_DAYS if direction == "long" else SHORT_HOLD_DAYS
            exit_dt = ed + timedelta(days=hold + 1)  # conservative slot release
            portfolio.append({"exit_date": exit_dt, "ticker": ticker, "direction": direction})
...
def plot_open_positions():
    ...
    percentile = counts_df["open"].quantile(0.75)
    ...
    print(f"  75th percentile open positions/day  : {percentile:.1f}")
